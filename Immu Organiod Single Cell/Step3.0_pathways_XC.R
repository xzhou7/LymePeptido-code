...
# Then install ggunchull from GitHub
#devtools::install_github("sajuukLyu/ggunchull")


#mac
#setwd("~/Library/CloudStorage/Dropbox/lyme_disease/peptiglygen_stimulation/Mono_LPS_Pepti_Sti/RNA-seq/")
setwd("~/Library/CloudStorage/Dropbox/lyme_disease/Pepti_Spleen_singlecell/")
getwd()

#doubel check the spleen_DEG-jUNE 15
spleen_DEG <- read.csv("../Manuscript/Figures/Figure 5-immune organoids and autoreactive B cells/Spleen_DEG.csv", header = T, row.names = 1)
...
Patient_DEG <- read.csv("~/Library/CloudStorage/Dropbox/lyme_disease/PTLD_HD_Monocytes/HD_PTLD.csv", header = T)
colnames(Patient_DEG)
...
head(Patient_DEG_sig_overlap$label)
...
filter(Peptido_DEG_PATH,SYMBOL == "CD83")


#MACROP-INCREASED
filter(Peptido_DEG_PATH,SYMBOL == "CXCL1")
filter(Peptido_DEG_PATH,SYMBOL == "CXCL2")
filter(Peptido_DEG_PATH,SYMBOL == "CXCL3")
filter(Peptido_DEG_PATH,SYMBOL == "CCL8")



#macro-reduced
filter(Peptido_DEG_PATH,SYMBOL == "TXNIP")
filter(Peptido_DEG_PATH,SYMBOL == "GIMAP7")
filter(Peptido_DEG_PATH,SYMBOL == "TSC22D3")

filter(Peptido_DEG_PATH,SYMBOL == "KIR2DL4")
...
B_up.kegg <- setReadable(B_up.DOSE, OrgDb = org.Hs.eg.db, keyType = "ENTREZID")

B_up_kegg_df <- as.data.frame(B_up.kegg)

# Write to CSV

#write.csv(
#  B_up_kegg_df,
#  file = "~/Library/CloudStorage/Dropbox/lyme_disease/Manuscript/Figures/Figure 5-immune organoids and autoreactive B cells/B_up_kegg_df_results.csv",
#  row.names = FALSE
#)
...
setwd("~/Library/CloudStorage/Dropbox/lyme_disease/Pepti_Spleen_singlecell/")
getwd()
#Prepare DEG data for each cell type
B_sig <-filter(Peptido_DEG_PATH, celltype == "B_Cell", p_val_adj < 0.05)
CD4_sig <-filter(Peptido_DEG_PATH, celltype == "CD4T_Cell", p_val_adj < 0.05)
CD8_sig <-filter(Peptido_DEG_PATH, celltype == "CD8T_Cell", p_val_adj < 0.05)
MacDC_sig <-filter(Peptido_DEG_PATH, celltype == "Macrophage_DC", p_val_adj < 0.05)
NK_sig <-filter(Peptido_DEG_PATH, celltype == "NK_Cell", p_val_adj < 0.05)


#Store DEGs in a list for all cell types
deg_list <- list(
  Macrophage_DC = MacDC_sig,
  CD8_T = CD8_sig,
  CD4_T = CD4_sig,
  NK = NK_sig,
  Bcell = B_sig
...
sig_up <- lapply(deg_list, function(df) {
  df %>% filter(avg_log2FC > 0.25) %>% pull(gene)
})

sig_down <- lapply(deg_list, function(df) {
  df %>% filter(avg_log2FC < -0.25) %>% pull(gene)
})


#Count overlaps across cell types
up_counts <- table(unlist(sig_up))
down_counts <- table(unlist(sig_down))

#Step 5: Get genes shared in ≥N cell types
genes_up_shared <- names(up_counts[up_counts >= 5])
genes_down_shared <- names(down_counts[down_counts >= 5])

#Remove genes that contain "ENSG"
genes_up_filtered <- genes_up_shared[!grepl("^ENSG", genes_up_shared)]

#Combine your gene list
genes_combined <- unique(c(genes_up_shared, genes_down_shared))
genes_combined <- genes_combined[!grepl("^ENSG", genes_combined)]  # remove ENSG genes

#if wanto add more gene: 
...
# Vector of genes you want to remove
genes_to_remove <- c("RPP21", "TSEN34", "RGL2", "PSMB8-AS1", "LENG1", "GPANK1", "CNOT3", "SAMHD1", "HLA-B", "LSM2", "RDH13", "SNHG32", "MDC1", "PRR3", "OSCAR", "FCGR3A" )

# Remove from your data frame (e.g., ALL_DEG)

genes_combined <- genes_combined[!genes_combined %in% genes_to_remove]
...
matched_peptido_genes_unique <- matched_peptido_genes %>%
  distinct(gene, celltype, .keep_all = TRUE)

# Pivot to a wide matrix (gene × celltype)
library(tidyr)

heatmap_df <- matched_peptido_genes_unique %>%
...
pheatmap(heatmap_mat,
         ...
         filename = output_file,       # <-- saves to file directly
         width = 4,
         height = 10)
