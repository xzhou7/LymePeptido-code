...
#ggsave(filename = "../Manuscript/Figures/Figure2/Figure2B.pdf", pdot, width = 11, height = 5, dpi = 300)
...
count2.2 <- subset(count2.2, Var2 %in% c("Activated_CD4","Naive_CD8","GZMB_CD8","CD56bright_NK","CD56dim_NK"))
count2.2$Var2 <- factor(count2.2$Var2, levels = c("Activated_CD4","Naive_CD8","GZMB_CD8","CD56bright_NK","CD56dim_NK"))


count2.2$Conditon <- factor(count2.2$Conditon, levels = c("RTH", "PTLD"))

comparasions <- list(c("RTH","PTLD"))
...
p2.2 <- ggplot(count2.2, aes(x = Conditon, y = Percent, color = Conditon)) +
  ...
  theme(
    ...
    axis.text.y = element_text(size = 12),  # optional: adjust y-axis label size
    ...
    axis.ticks.length = unit(0.2, "cm")          # control tick size
  )

p2.2

ggsave(filename = "/Users/xinchen/Library/CloudStorage/Dropbox/lyme_disease/Manuscript/Figures/Figure2/Fig2C_Percentage.pdf",p2.2, width = 10, height = 4, dpi = 300 )
...
#ggsave(filename = "/Users/xinchen/Library/CloudStorage/Dropbox/lyme_disease/Manuscript/Figures/Figure2/Figure4B_Treg.pdf",p.treg, width = 5, height = 5, dpi = 300)
...
antibody.list
#FeaturePlot(T.NK.clean_Annotated,"CD14.MPHIP9.CD14.AHS0037.pAbO", raster=T, max.cutoff = 50)
...
#######May31
AC_CD4.clean <- subset(T.NK.clean_Annotated, idents= c("Activated_CD4"))
...
FeaturePlot(T.NK.clean_Annotated, features = "LAIR2")
...
upregulated__PTLDN <- CD4_PTLDN_HD[CD4_PTLDN_HD$avg_log2FC > 0, ]
upregulated__PTLD <- CD4_PTLD_RTH[CD4_PTLD_RTH$avg_log2FC > 0, ]

upregulated__PTLD$avg_log2FC <- -upregulated__PTLD$avg_log2FC

combined_list <- rbind.data.frame(upregulated__PTLDN, upregulated__PTLD)
...
slice3.VP_CD4 <- EnhancedVolcano(CD4_PTLDN_HD,
                                 ...
                                 title = "CD4_activated")
combine
slice1.VP_CD4 <- EnhancedVolcano(CD4_PTLD_RTH,
                                 lab = CD4_PTLD_RTH$gene,
                                 FCcutoff=0.15,
                                 xlim = c(-2.3, 2.3),
                                 x = 'avg_log2FC',
                                 y = 'p_val_adj', 
...
slice1.VP_CD4
slice3.VP_CD4 <- EnhancedVolcano(CD4_PTLD_RTH,
                                 lab = CD4_PTLD_RTH$gene,
                                 FCcutoff=0.15,
                                 xlim = c(-2.3, 2.3),
                                 x = 'avg_log2FC',
...
# Search for the gene DUSP4 in the gene column
...
PTLDN_HD_ALL[PTLDN_HD_ALL$gene == "CD83", ]
CD4_PTLDN_HD[CD4_PTLDN_HD$gene == "DUSP4", ]
CD8_PTLDN_HD[CD8_PTLDN_HD$gene == "DUSP4", ]
Treg_PTLDN_HD[Treg_PTLDN_HD$gene == "DUSP4", ]
GammaDelta_T_PTLDN_HD[GammaDelta_T_PTLDN_HD$gene == "DUSP4", ]
TFH_PTLDN_HD[TFH_PTLDN_HD$gene == "DUSP4", ]
MAIT_PTLDN_HD[MAIT_PTLDN_HD$gene == "DUSP4", ]
...
######Figure2 还有点问题
#install.packages("readxl")  # Run this once if not already installed
library(readxl)
library(dplyr)
...
p <- filter(PTLDN_HD_ALL,p_val_adj < 0.05) %>%  ggplot(aes(x = celltype, y = avg_log2FC, label = gene)) +
  ...
  geom_text_repel(data = subset(PTLDN_HD_ALL, (gene %in% Treg_genes & celltype == "Treg")), vjust = -1, hjust = 0.5, color = "black", size = 5) +
  geom_text_repel(data = subset(PTLDN_HD_ALL, (gene %in% CD8_T_genes  & celltype == "Memory_CD8")), vjust = -1, hjust = 0.5, color = "black", size = 5) +
  geom_text_repel(data = subset(PTLDN_HD_ALL, (gene %in% gammaDelta_T_genes  & celltype == "GammaDelta_T")), vjust = -1, hjust = 0.5, color = "black", size = 5) +
  geom_text_repel(data = subset(PTLDN_HD_ALL, (gene %in% MAIT_genes  & celltype == "MAIT")), vjust = -1, hjust = 0.5, color = "black", size = 5) +
  geom_text_repel(data = subset(PTLDN_HD_ALL, (gene %in% CD4_T_genes  & celltype == "Memory_CD4")), vjust = -1, hjust = 0.5, color = "black", size = 5) +
  geom_text_repel(data = subset(PTLDN_HD_ALL, (gene %in% NK_genes  & celltype == "NK")), vjust = -1, hjust = 0.5, color = "black", size = 5) +
  geom_text_repel(data = subset(PTLDN_HD_ALL, (gene %in% TFH.gene  & celltype == "TFH")), vjust = -1, hjust = 0.5, color = "black", size = 5) +
  ...
  coord_cartesian(ylim = c(-3, 3)) +  # <-- This line zooms in on y-axis
  XZ_flip_x_label() +
  guides(color = FALSE)
p  

ggsave(filename = "../Manuscript/Figures/Figure2/Gene.List_v3.pdf", p, width = 12, height = 8, dpi=300)
